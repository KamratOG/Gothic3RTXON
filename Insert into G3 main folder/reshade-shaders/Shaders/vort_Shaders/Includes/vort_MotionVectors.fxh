...
static const float2 block_offs[13] =
{
    float2(0, 0),
...
float4 CalcLayer(VSOUT i, int mip, float2 total_motion)
{
    ...
    int block_samples = mip > 3 ? 13 : 9;
    float inv_block_samples = 1.0 / float(block_samples);
    ...
    float best_sim = moments_cov * RSQRT(moments_local * moments_search);
    ...
    [loop]while(searches-- > 0 && best_sim < max_sim)
    {
        ...
        [loop]while(samples-- > 0 && best_sim < max_sim)
        {
            ...
            float sim = moments_cov * RSQRT(moments_local * moments_search);
...
{
    uint feature_mip = max(0, mip - MIN_MIP);
    float2 scale = rcp(tex2Dsize(mot_samp)) * (mip > 0 ? 6.0 : 1.5);
    float2 rand = GetR2(GetBlueNoise(i.vpos.xy).xy, mip + MIN_MIP) - 0.5;
    float center_z = Sample(sDownDepthTexVort, i.uv, feature_mip).x;
    ...
    {
        float2 sample_uv = i.uv + (float2(x, y) + rand) * scale;
        float4 sample_gbuf = Sample(mot_samp, sample_uv);
        float sample_z = Sample(sDownDepthTexVort, sample_uv, feature_mip).x;

        float wz = abs(center_z - sample_z) * RCP(min(center_z, sample_z)); wz *= wz * 32.0; // depth delta
        float wm = dot(sample_gbuf.xy, sample_gbuf.xy) * 250.0; // long motion
        float ws = sample_gbuf.w; // similarity
        float weight = max(0.001, exp2(-(wz + wm + ws) * 4.0));
...
void PS_Motion5(PS_ARGS4) { o = EstimateMotion(i, 5, sMotionTexVortA); }
void PS_Motion4(PS_ARGS4) { o = EstimateMotion(i, 4, sMotionTexVortB); }
void PS_Motion3(PS_ARGS4) { o = EstimateMotion(i, 3, sMotionTexVortA); }
void PS_Motion2(PS_ARGS4) { o = EstimateMotion(i, 2, sMotionTexVortB); }
void PS_Motion1(PS_ARGS4) { o = EstimateMotion(i, 1, sMotionTexVort2); }
